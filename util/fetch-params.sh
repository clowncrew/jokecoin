...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/JokeCoinParams"
    else
        PARAMS_DIR="$HOME/.jokecoin-params"
    fi
fi
...
Failed to fetch the JokeCoin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
JokeCoin - fetch-params.sh

This script will fetch the JokeCoin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common JokeCoin zkSNARK parameters. Note that it is
