...
class ReorgStakeTest(JokeCoinTestFramework):
    ...
    def check_money_supply(self, expected_joke, expected_zjoke):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected JOKE and zJOKE supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_joke))
            for denom in node['zJOKEsupply']:
                assert_equal(node['zJOKEsupply'][denom], DecimalAmt(expected_zjoke[denom]))
    ...
    def run_test(self):
        ...
        # Check JOKE and zJOKE supply at the beginning
        # ------------------------------------------
        # zJOKE supply: 2 coins for each denomination
        expected_zjoke_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # JOKE supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zjoke_supply)
        ...
        # Verify that JOKE and zJOKE supplies were properly updated after the spends and reorgs
        self.log.info("Check JOKE and zJOKE supply...")
        ...
        expected_zjoke_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zjoke_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zjoke_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zjoke_supply)
