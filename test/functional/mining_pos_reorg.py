...
class ReorgStakeTest(JokeCoinTestFramework):
    ...
    def check_money_supply(self, expected_joke):
        # verify that nodes have the expected JOKE supply
        joke_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(joke_supply, [DecimalAmt(expected_joke)] * self.num_nodes)
