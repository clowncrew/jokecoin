...
class JokeCoinTestFramework():
    """Base class for a jokecoin test script.

    Individual jokecoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def _initialize_chain(self, toPosPhase=False):
        ...
        def copy_cachedir(origin, destination, num_nodes=MAX_NODES):
            for i in range(num_nodes):
                ...
                initialize_datadir(destination, i)  # Overwrite port/rpcport in jokecoin.conf
        ...
        def start_nodes_from_dir(ddir, num_nodes=MAX_NODES):
            ...
            for i in range(num_nodes):
                ...
                args = [os.getenv("BITCOIND", "jokecoind"), "-spendzeroconfchange=1", "-server", "-keypool=1",
        ...
        if not toPosPhase and not is_powcache_valid:
            ...
            # - Nodes 2 and 3 mint one zerocoin for each denom (tot 6666 JOKE) on block 301/302
            ...
            for peer in range(4):
                for j in range(20):
                    ...
                    if nBlocks == 301 or nBlocks == 302:
                        # mints 7 zerocoins, one for each denom (tot 6666 JOKE), fee = 0.01 * 8
                        # consumes 27 utxos (tot 6750 JOKE), change = 6750 - 6666 - fee
    ...
    ### JokeCoin Specific TestFramework ###
    ###################################
    ...
    def test_PoS_chain_balances(self):
        ...
        # - Nodes 2 and 3 have 6666 JOKE worth of zerocoins
    ...
    def get_prevouts(self, node_id, utxo_list, zpos=False, nHeight=-1):
        ...
        :return: prevouts:         ({bytes --> (int, bytes, int)} dictionary)
                                   maps CStake "uniqueness" (i.e. serialized COutPoint -or hash stake, for zjoke-)
                                   to (amount, prevScript, timeBlockFrom).
                                   For zjoke prevScript is replaced with serialHash hex string.
        """
    ...
    def stake_block(self, node_id,
        ...
        for tx in vtx:
            if not fDoubleSpend:
                # assume txes don't double spend zJOKE inputs when fDoubleSpend is false. It needs to
