...
class ZerocoinSpendTest(JokeCoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zjoke_bal, joke_bal):
            zjoke_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zjoke_bal)
            joke_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], joke_bal)
            return zjoke_bal, joke_bal
        ...
        zjoke_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zjoke_balance, 6666)
        ...
        zjoke_balance, balance = check_balances(denom_2, zjoke_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zjoke_balance, balance = check_balances(denom_3, zjoke_balance, balance)
        ...
        _, _ = check_balances(denom_1, zjoke_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
