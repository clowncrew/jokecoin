JokeCoin Core version *v5.0.0* is now available from:  <https://github.com/jokecoin-project/jokecoin/releases>
...
JokeCoin Core v5.0.0 is a mandatory update for all users. This release contains a protocol upgrade (to `70920`), new consensus rules and improvements that are not backwards compatible with older versions. Users will need to update their clients before enforcement of this update goes into effect.
...
#### Overview
Users will be able to protect their financial information by sending and receiving JOKEs privately, as well as sending and receiving encrypted messages attached to the shield transactions.
More visual information about the protocol can be found at https://jokecoin.org .
...
* Top bar showing shield JOKE balances.
...
* New derivation path for Sapling keys: Shield addresses are derived from the same wallet master seed used for the deterministic derivation of all other keys. The same seed, therefore, can be used to restore both transparent and Shield addresses, recovering both transparent and Shield JOKE balance.
...
See the release notes from [v4.2.0](https://github.com/JokeCoin-Project/JokeCoin/blob/master/doc/release-notes/release-notes-4.2.0.md#label-and-account-apis-for-wallet) for a full description of the changes from the 'account' API to the 'label' API.
...
| `getsupplyinfo` | Returns detailed JOKE supply information | No |
...
* `viewshieldtransaction`
  ...
  Result:
  {
    ...
    "fee"  : x.xxx,             (numeric) The transaction fee in JOKE
    "spends" : [
      {
        ...
        "value" : x.xxx                 (numeric) The amount in JOKE
...
* `getinfo`
  The `zerocoinbalance` JSON return entry and the `zJOKEsupply` return JSON object have been removed due to zerocoin deprecation.
  The `balance` JSON return entry is now all inclusive (the sum of all transparent and shield JOKE)
  The `moneysupply` JSON return entry is the total JOKE supply (the sum of all transparent and shield JOKE)
...
| `getaccountaddress` | Returned the current JokeCoin address associated with an internal account | Account system removal |
| `setaccount` | Associate a JokeCoin address with an internal account name | Account system removal |
| `getaccount` | Returned the internal account associated with a given JokeCoin address | Account system removal |
| `getaddressesbyaccount` | Returned a list of JokeCoin addresses associated with an internal account | Account system removal |
...
| `sendfrom` | Send an amount from an internal account to a JokeCoin address | Account system removal |
...
| `multisend` | Configure sending a portion of stake rewards to a JokeCoin address | Pending a full rewrite |
| `getzerocoinbalance` | Returned the wallet's total zJOKE balance | Zerocoin deprecation |
| `listmintedzerocoins` | List all zJOKE mints in the wallet | Zerocoin deprecation |
| `listzerocoinamounts` | List information about your zerocoin amounts | Zerocoin deprecation |
| `listspentzerocoins` | List all the spent zJOKE mints in the wallet | Zerocoin deprecation |
| `mintzerocoin` | Mint the specified zJOKE amount | Zerocoin deprecation |
| `spendzerocoin` | Spend zJOKE to a JOKE address | Zerocoin deprecation |
| `spendzerocoinmints` | Spend specific zJOKE mints to a JOKE address | Zerocoin deprecation |
...
| `reconsiderzerocoins` | Check archived zJOKEs to see if any mints were added to the chain | Zerocoin deprecation |
| `setzjokeseed` | Set the wallet's deterministic zJOKE seed to a specific value | Zerocoin deprecation |
| `getzjokeseed` | Return the current deterministic zJOKE seed | Zerocoin deprecation |
| `generatemintlist` | Show mints that are derived from the deterministic zJOKE seed | Zerocoin deprecation |
| `dzjokestate` | Show the current state of the mintpool of the wallet | Zerocoin deprecation |
| `searchdzjoke` | Do a search for deterministically generated zJOKE that have not yet added to the wallet | Zerocoin deprecation |
...
### Build Systems
- #1856 `1bbb906059` [CMake] Add test_jokecoin unit test target (Fuzzbawls)
...
- #2041 `a15dc3417d` [Build] CMake jokecoind and unit tests: fixing missing MacOS framework. (furszy)
