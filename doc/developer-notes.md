...
- [Development guidelines](#development-guidelines)
    - [General JokeCoin Core](#general-jokecoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
JokeCoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play JOKEs (tJOKE)" on the test network, if you
...
JokeCoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/jokecoin-project/jokecoin/blob/master/contrib/valgrind.supp))
...
**Sanitizers**
JokeCoin can be compiled with various "sanitizers" enabled, which add
...
- BitcoinMiner : Generates JOKEs (if wallet is enabled).
...
However, in open source software such as JokeCoin Core, where everyone uses
...
pay attention to for reviewers of JokeCoin Core code.

General JokeCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: JokeCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against JokeCoin Core so that they can be integrated
...
file descriptors it will cause problems with JokeCoin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:JokeCoin-Project/JokeCoin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `jokecoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `jokecoin-cli` and the GUI debug console use this table to determine how to
