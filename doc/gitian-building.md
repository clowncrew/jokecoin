...
*Setup instructions for a gitian build of JokeCoin Core using a VM or physical system.*
...
to the JokeCoin GitHub release page.
...
- [Building JokeCoin Core](#building-jokecoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/jokecoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/jokecoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/jokecoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/jokecoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/jokecoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/jokecoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/jokecoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@jokecoin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/jokecoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/jokecoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/jokecoin-osx-*-build.assert
```
