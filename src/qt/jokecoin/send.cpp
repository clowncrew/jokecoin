...
#include "qt/jokecoin/send.h"
#include "qt/jokecoin/forms/ui_send.h"
#include "qt/jokecoin/addnewcontactdialog.h"
#include "qt/jokecoin/qtutils.h"
#include "qt/jokecoin/sendchangeaddressdialog.h"
#include "qt/jokecoin/optionbutton.h"
#include "qt/jokecoin/sendconfirmdialog.h"
#include "qt/jokecoin/guitransactionsutils.h"
#include "qt/jokecoin/loadingdialog.h"
...
SendWidget::SendWidget(JokeCoinGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-joke");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onJOKESelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onJOKESelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent JOKEs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield JOKEs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent JOKEs to shield."));
    }
}
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto jokeAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(jokeAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(jokeAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(jokeAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
