...
#include "qt/jokecoin/forms/ui_dashboardwidget.h"
#include "qt/jokecoin/sendconfirmdialog.h"
#include "qt/jokecoin/txrow.h"
...
DashboardWidget::DashboardWidget(JokeCoinGUI* parent) :
    PWidget(parent),
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &JokeCoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &JokeCoinGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &JokeCoinGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair JOKE, zJOKE
