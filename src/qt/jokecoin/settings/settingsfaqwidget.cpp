...
#include "qt/jokecoin/settings/forms/ui_settingsfaqwidget.h"
#include <QScrollBar>
#include <QMetaObject>
#include "qt/jokecoin/qtutils.h"

SettingsFaqWidget::SettingsFaqWidget(JokeCoinGUI *parent) :
    QDialog(parent),
    ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({
           ...
           ui->labelNumber_UnspendableJOKE,
    ...
    setCssProperty({
              ...
              ui->labelSubtitle_UnspendableJOKE,
    ...
    setCssProperty({
              ...
              ui->labelContent_UnspendableJOKE,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("JokeCoin is a form of digital online money using blockchain technology "
        ...
        formatFAQParagraph(
            tr("JokeCoin utilizes a Proof of Stake (PoS) consensus system algorithm, "
    ...
    QString unspendableJOKEContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received JokeCoin requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.")) +
        formatFAQParagraph(
            tr("Your JokeCoin wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableJOKE->setText(unspendableJOKEContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of JokeCoin with a minimum of 600 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("jokecoin-core", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in most of our official chat groups, for example %1")
                .arg("<a style='color: #b088ff' href='https://discord.JokeCoin.org'>" + tr("#support in our Discord") + "</a>.")));
    ui->labelContent_Support->setText(supportContent);

    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node JokeCoin core wallet with a "
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in JokeCoin Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the JokeCoin network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("10,000 JOKE per single Masternode instance")) +
    ...
    QString mNControllerContent = formatFAQContent(
        formatFAQParagraph(
            tr("A Masternode Controller wallet is where the 10,000 JOKE collateral "
    ...
    connect(ui->pushButton_UnspendableJOKE, &QPushButton::clicked, [this](){onFaqClicked(ui->widget_UnspendableJOKE);});
    ...
    if (parent)
        connect(parent, &JokeCoinGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}
