...
#include "qt/jokecoin/forms/ui_snackbar.h"
#include "qt/jokecoin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(JokeCoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &JokeCoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
