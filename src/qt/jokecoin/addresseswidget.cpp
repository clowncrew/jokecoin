...
#include "qt/jokecoin/forms/ui_addresseswidget.h"
#include "qt/jokecoin/addresslabelrow.h"
#include "qt/jokecoin/addnewaddressdialog.h"
#include "qt/jokecoin/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(JokeCoinGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto jokeAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(jokeAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(jokeAdd);
        if (walletModel->updateAddressBookLabels(jokeAdd, label.toUtf8().constData(),
