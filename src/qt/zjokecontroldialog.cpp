...
#include "ui_zjokecontroldialog.h"
...
std::set<std::string> ZJokeControlDialog::setSelectedMints;
std::set<CMintMeta> ZJokeControlDialog::setMints;

bool CZJokeControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZJokeControlDialog::COLUMN_DENOMINATION || column == ZJokeControlDialog::COLUMN_VERSION || column == ZJokeControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZJokeControlDialog::ZJokeControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZJokeControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZJokeControlDialog::~ZJokeControlDialog()
{
    delete ui;
}

void ZJokeControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZJokeControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZJokeControlWidgetItem* itemDenom(new CZJokeControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZJokeControlWidgetItem *itemMint = new CZJokeControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zjokeTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zjokeTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zJOKE.";
            else if (!mint.isSeedCorrect)
                strReason = "The zJOKE seed used to mint this zJOKE is not the same as currently hold in the wallet";
            else
...
void ZJokeControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZJokeControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZJoke_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZJokeControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZJokeControlDialog::GetSelectedMints()
{
...
void ZJokeControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
