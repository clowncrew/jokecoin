...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::JOKE);
    ...
    if (settings.contains("nAnonymizeJokeCoinAmount") || reset)
        gArgs.SoftSetArg("-anonymizejokecoinamount", settings.value("nAnonymizeJokeCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in jokecoin.cpp during shotdown to also remove the window positions
