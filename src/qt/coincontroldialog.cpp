...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select JOKE Outputs to Spend" :
            "Select Shielded JOKE to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::JOKE;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show JokeCoin address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
