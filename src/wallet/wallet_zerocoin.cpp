    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZJOKESeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZJOKESeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zjokeseed to wallet";
    }
    //the use case for this is no password set seed, mint dzJOKE,
...
bool CWallet::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZJOKESeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZJOKESeed(hashSeed, vchSeed)) {
        ...
        std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZJOKE)
{
    ...
    if (fCheckZJOKE && consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC)) {
...
bool CWallet::CreateZJOKEOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwallet->GenerateDeterministicZJOKE(denomination, coin, dMint);
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    for (auto &it : mapMintsSelected) {
        ...
        CTxIn in;
        if(!ZJOKEModule::createInput(in, mint, hashTxOut, spendVersion)) {
        ...
        CTxDestination * changeAddress)
{
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zJOKE public spend transaction
...
CAmount CWallet::GetZerocoinBalance(bool fMatureOnly) const
{
    if (fMatureOnly) {
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zjokeTracker->GetMints(true);
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zjokeTracker->ListMints(true, true, true);
...
void CWallet::setZWallet(CzJOKEWallet* zwallet)
{
    this->zwallet = zwallet;
    zjokeTracker = std::unique_ptr<CzJOKETracker>(new CzJOKETracker(this));
}

CzJOKEWallet* CWallet::getZWallet()
{
    return zwallet;
}
