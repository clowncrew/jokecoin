...
CzJOKEWallet::CzJOKEWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zjoke seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZJOKESeed_deprecated(seed)) {
            ...
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZJOKESeed_deprecated()) {
                    ...
                    LogPrintf("%s: failed to remove old zjoke seed\n", __func__);
                }
            }
        }
    }
...
bool CzJOKEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    else if (!walletdb.ReadZJOKECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzJOKEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
...
void CzJOKEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzJOKEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzJOKEWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzJOKEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZJOKE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
uint512 CzJOKEWallet::GetZerocoinSeed(uint32_t n)
{
...
bool CzJOKEWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzJOKEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
