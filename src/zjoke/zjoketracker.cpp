// Copyright (c) 2018-2019 The JokeCoin developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zjoke/deterministicmint.h>
#include "zjoketracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzJOKETracker::CzJOKETracker(std::string strWalletFile)
{
...
CzJOKETracker::~CzJOKETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzJOKETracker::Init()
{
...
bool CzJOKETracker::Archive(CMintMeta& meta)
{
...
bool CzJOKETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzJOKETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzJOKETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzJOKETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzJOKETracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzJOKETracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzJOKETracker::GetSerialHashes()
{
...
CAmount CzJOKETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzJOKETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzJOKETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzJOKETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzJOKETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzJOKETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzJOKETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzJOKETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzJOKETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzJOKETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzJOKETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzJOKEWallet* zJOKEWallet)
{
    bool iszJOKEWalletInitialized = (NULL != zJOKEWallet);
    CMintMeta meta;
    ...
    if (! iszJOKEWalletInitialized)
        zJOKEWallet = new CzJOKEWallet(strWalletFile);
    meta.isSeedCorrect = zJOKEWallet->CheckSeed(dMint);
    if (! iszJOKEWalletInitialized)
        delete zJOKEWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzJOKETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzJOKETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzJOKETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzJOKETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzJOKETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzJOKETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzJOKEWallet* zJOKEWallet = new CzJOKEWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zJOKEWallet);
        }
        delete zJOKEWallet;
        LogPrint("zero", "%s: added %d dzjoke from DB\n", __func__, listDeterministicDB.size());
    }
