...
BOOST_AUTO_TEST_CASE(MarkAffectedSaplingTransactionsDirty) {
    ...
    // 0.5 t-JOKE in, 0.4 z-JOKE out, 0.1 t-JOKE fee
    ...
    // 0.4 z-JOKE in, 0.25 z-JOKE out, 0.1 t-JOKE fee, 0.05 z-JOKE change
    auto builder2 = TransactionBuilder(consensusParams, 2);
...
BOOST_AUTO_TEST_CASE(GetNotes)
{
    ...
    std::vector<SaplingOutPoint> saplingOutpoints;
    {
        ...
        // Generate shielding tx from transparent to Sapling (five 1 JOKE notes)
        auto builder = TransactionBuilder(consensusParams, 1, &keystore);
