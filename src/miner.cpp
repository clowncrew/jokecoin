...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake, std::vector<COutput>* availableCoins)
{
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            // zJOKE check to not include duplicated serials in the same block.
...
void BitcoinMiner(CWallet* pwallet, bool fProofOfStake)
{
    ...
    util::ThreadRename("jokecoin-miner");
    const Consensus::Params& consensus = Params().GetConsensus();
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        } else if (consensus.NetworkUpgradeActive(((pindexPrev->nHeight <= 6) ? 0 : (pindexPrev->nHeight - 6)), Consensus::UPGRADE_POS)) {
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("JokeCoinMiner exception");
    } catch (...) {
        LogPrintf("JokeCoinMiner exception");
    }

    LogPrintf("JokeCoinMiner exiting\n");
}
