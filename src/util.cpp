...
const char * const JokeCoin_CONF_FILENAME = "jokecoin.conf";
const char * const JokeCoin_PID_FILENAME = "jokecoin.pid";
const char * const JokeCoin_MASTERNODE_CONF_FILENAME = "masternode.conf";


// JokeCoin only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\JokeCoin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\JokeCoin
// Mac: ~/Library/Application Support/JokeCoin
// Unix: ~/.jokecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JokeCoin";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\JokeCoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\JokeCoinParams
    // Mac: ~/Library/Application Support/JokeCoinParams
    // Unix: ~/.jokecoin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "JokeCoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "JokeCoinParams";
#else
    // Unix
    return pathRet / ".jokecoin-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/jokecoin/sapling-spend.params";
        sapling_output = "/usr/share/jokecoin/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/jokecoin/sapling-spend.params";
            sapling_output = "/usr/local/share/jokecoin/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", JokeCoin_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", JokeCoin_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty jokecoin.conf if it does not exist
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override jokecoin.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", JokeCoin_PID_FILENAME));
